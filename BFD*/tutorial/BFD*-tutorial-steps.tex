...
\step{Downloading the programs.}{
    ...
    This tutorial is written for the Mac OS X version of \program{BEAST}v2.4.1.
    
    You will be using \program{BEAST} to run \program{SNAPP}, although it is possible to run \program{SNAPP} on its own. However, you 
    ...
    Thus, without \program{BEAST} you would not be able to conduct species delimitation with \program{SNAPP}.
    }
...
\step{Data included with the tutorial.}{
   After downloading and unzipping this archive you should have a BFD*-tutorial folder on your computer. This tutorial
    ...
    SNP data from nucleotide to binary format, please read the documentation \href{http://beast2.org/tutorials/}{A rough guide to SNAPP} (Section 4. Preparing Input File). You can find scripts for converting SNP data into \program{SNAPP} input format at our phrynomics project site at \href{https://github.com/bbanbury/phrynomics}{GitHub}. You can also find help
...
\step{Install SNAPP and model selection packages}{You need to add functionality to \program{BEAST} in order to estimate 
...
\step{Converting \program{BEAUTi} to \program{SNAPP} mode}{Tell \program{BEAUTi} that you are  
...
\step{Set the mutation model.}{
    ...
    Be sure to read the documentation \href{http://www.beast2.org/tutorials}{A rough guide to SNAPP} to learn more about the model options. Briefly, the parameters are as follows:
...
\small{
    ...
    different if \program{SNAPP} assumes that all constant sites have been removed. If you are using a typical SNP dataset that only includes variable site, then make sure that the box is not checked.
...
\step{Define the priors.}{
    ...
    One problem with \program{SNAPP} is that it is deceptively easy to set up an analysis using default options. 
...
If for some reason you are using an older versions of \program{SNAPP} (<v1.1.10) you may need an extra step; find the text {\bf ``snap.MCMC''} and replace it with {\bf ``beast.core.MCMC''.}  
...
Table 2: Path sampling results for the seven species delimitation models shown in Figure \ref{fig:map}. All Bayes factor (BF) calculations are made against the current taxonomy model (runA). Therefore, negative BF values indicate support for the current taxonomy model, and positive BF values indicate support for the alternative model.
\begin{table}[ht]
\tabcolsep=0.2cm
...
TreeAnnotator will summarize the posterior distribution of species trees and identify the topology with the best posterior support, and summarize the divergence times for each node in the tree.
	...
	Select the \field{Input Tree File} button and select the file \localfile{runA.trees}. When conducting path sampling, SNAPP generates a posterior distribution of trees for each path sampling step. You will typically only want to summarize the tree file in the folder corresponding to theta=1, since the other folders contain trees that were estimated with modified likelihoods. Check the SNAPP screen output to verify which path sampling step corresponds to theta=1 (this changes with different versions of SNAPP).
